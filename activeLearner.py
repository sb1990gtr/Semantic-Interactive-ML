# -*- coding: utf-8 -*-
"""
Created on Fri Dec  3 10:02:16 2021

@author: Mareike Hoffmann
"""
#README: Implementation of modAL Active Learner
#The learner works with a XGBoost Classifier
#The most "useful" example to be labeled is measured by uncertainty sampling method
#The basic active learning process - labeling the examples - 
#is expanded by adding new, customized and artificially generated training data
# TODO 1) prüfe korrekte Funtionsweise des GS / local GS für CAIPI (SI_LIME_dest) word based: Warum wird GS local im Nachhinein eingeschränkt?
# TODO 2) prüfe korrekte Funktionsweise des GS / local GS für SI_LIME_const word based: Ist alles fair im Vergleich zu den topics? Anzahl Erklärungen und Anzahl der Elemente des GS?
# TODO 3) wieiviele Erklärungen in den einzelnen Modalitäten?
# TODO 4) Prüfe korrekte Funktionsweise der Evaluation der Erklärungen
# TODO 5) paper?
...
#Parameters for S2)-S4): provide the true label and extra training documents
...
while iteration < 200:
    ...
    #S3)
    #TopicLIME
    #if prediction is true, correct the explanation if necessary
    #if prediction is false, correct the prediction
    # if prediction == [y_data_i[global_id]]:
    #     #correct the original instance
    #     learner.teach(x_pool[query_idx], y_pool[query_idx])
    #     x_correction_i, kl_pq = TopicLIME_SI_dest(global_id, prediction, query_inst, learner, alpha_TL, num_docs)
    #     y_correction_i = [y_data_i[global_id]] * num_docs
    #     learner.teach(x_correction_i, y_correction_i)
    ...
    #     #correct the original instance
    #     learner.teach(x_pool[query_idx], y_pool[query_idx])
    #     x_correction_i, kl_pq = TopicLIME_SI_const(global_id, query_inst, learner, alpha_TL, num_docs, num_words_per_doc, alpha_SI_create)
    #     y_correction_i = [y_data_i[global_id]] * num_docs
    #     learner.teach(x_correction_i, y_correction_i)
    ...
    #
    # gs_model_topic = pickle.load(open(gs_path_TL, "rb"))
    # lda_model = LdaModel.load(lda_path)
    #
    # lda_corpus = corpora.MmCorpus(main_path + "/LDA/" + "masterthesiscorpus_complete")
    # print("lda_corpus at query: " + str(lda_corpus[query_idx]))
    # top_topics = lda_model.get_document_topics(lda_corpus[query_idx], minimum_probability=0.0)
    # print("test" + str(top_topics))
    # topic_vec = [top_topics[j][1] for j in range(lda_model.num_topics)]
    # vec = np.array(topic_vec)
    ...
    ######################### Evaluation ###################################
    # print("right: ", right)
    # #kullback leibler values of new trainings documents
    # #will be empty for LIME and Base Case approach
    # print("kullback leibler destructive documents: ", kullback_leibler_pq_dest)
    # print("kullback leibler constructive documents: ",kullback_leibler_pq_const)
    # print("kullback leibler counterexample documents: ", kullback_leibler_pq_counterexample)
    #
    # #RQ1 Trainset Evaluation
    # report_trainset = metrics.classification_report(learner.y_training, learner.predict(learner.X_training), output_dict=True)
    # #weighted_avg_trainset.append(report_trainset["weighted avg"]["f1-score"])
    # macro_avg_trainset.append(report_trainset["macro avg"]["f1-score"])
    # #accuracy_trainset.append(report_trainset["accuracy"])
    # #print("RQ1 weighted f1s_trainset ", weighted_avg_trainset)
    # print("RQ1 macro f1s_trainset ", macro_avg_trainset)
    # #print("RQ1 accuracy_trainset ", accuracy_trainset)
    ...
    macro_avg_testset.append(report_testset["macro avg"]["f1-score"])
    #accuracy_testset.append(report_testset["accuracy"])
    #print("RQ1 weighted f1s_testset ", weighted_avg_testset)
    print("RQ1 macro f1s_testset ", macro_avg_testset)
    #print("RQ1 accuracy_testset ", accuracy_testset)

    #RQ1 Number of correct predictions (TP)
    # confusion_matrix_trainset = metrics.confusion_matrix(learner.y_training, learner.predict(learner.X_training))
    # confusion_matrix_testset = metrics.confusion_matrix(y_test_i, learner.predict(x_test_i))
    # tp_confusion_matrix_trainset = int(np.trace(confusion_matrix_trainset))
    # tp_confusion_matrix_testset = int(np.trace(confusion_matrix_testset))
    # len_x_training = int(learner.X_training.shape[0])
    # len_x_test = int(x_test_i.shape[0])
    # correct_predictions_trainset.append(tp_confusion_matrix_trainset/len_x_training)
    # correct_predictions_testset.append(tp_confusion_matrix_testset/len_x_test)
    # print("RQ1 number of correct explanations trainset ", correct_predictions_trainset)
    # print("RQ1 number of correct explanations testset ", correct_predictions_testset)
